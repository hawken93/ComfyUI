...
class MMDiTJointAttention(nn.Module):
    ...
    def forward(self, x, y, pos_img, pos_txt=None, attn_mask=None, transformer_options={}):
        ...
        out_joint = self.optimized_attention(
...
class MMDiTBlockT2I(nn.Module):
    def __init__(self, device, operations, hidden_size, groups, mlp_ratio=4.0, dtype=None):
        super().__init__()
        ...
        self.attn = MMDiTJointAttention(device, operations, hidden_size, num_heads=groups, qkv_bias=False, dtype=dtype)
        ...
        self.mlp_x = FeedForwardSwiGLU(device, operations, hidden_size, mlp_hidden_dim, multiple_of=1, dtype=dtype)
        self.mlp_y = FeedForwardSwiGLU(device, operations, hidden_size, mlp_hidden_dim, multiple_of=1, dtype=dtype)
...
class PixDiT_T2I(nn.Module):
    ...
    def __init__(
        ...
        self.pixel_embedder = PixelTokenEmbedder(device, operations, self.in_channels, self.pixel_hidden_size, dtype=dtype)
        self.s_embedder = PatchTokenEmbedder(device, operations, self.in_channels * self.patch_size ** 2, self.hidden_size, bias=True, dtype=dtype)
        self.t_embedder = TimestepEmbedder(device, operations, self.hidden_size, dtype=dtype, max_period=10)
        self.y_embedder = PatchTokenEmbedder(device, operations, self.txt_embed_dim, self.hidden_size, bias=True, use_norm=True, dtype=dtype)
        self.y_pos_embedding = nn.Parameter(torch.empty(1, self.txt_max_length, self.hidden_size, dtype=dtype, device=device))

        self.patch_blocks = nn.ModuleList([
            MMDiTBlockT2I(device, operations, self.hidden_size, self.num_groups,
                          dtype=dtype)
        ...
        self.final_layer = FinalLayer(device, operations, self.pixel_hidden_size, self.out_channels, dtype=dtype)
